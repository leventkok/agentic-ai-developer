...
import (
	...
	"learn/go/day-54/internal/config"
	"learn/go/day-54/internal/db"
	"learn/go/day-54/internal/handler"
	"learn/go/day-54/internal/middleware"
...
func main() {
	...
	requireAdmin := middleware.RequireRole(auth.RoleAdmin)
	authRateLimit := middleware.NewRateLimiter(cfg.AuthRateLimitPerMinute, time.Minute)

	mux := http.NewServeMux()
	mux.Handle("POST /auth/register", authRateLimit.Limit(http.HandlerFunc(authHandler.Register)))
	mux.Handle("POST /auth/login", authRateLimit.Limit(http.HandlerFunc(authHandler.Login)))
	...
	mux.Handle("POST /bookmarks/bulk", requireAuth(requireAdmin(http.HandlerFunc(h.BulkCreateBookmarks))))

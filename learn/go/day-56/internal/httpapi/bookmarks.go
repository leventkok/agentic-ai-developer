...
import (
	...
	"learn/go/day-56/internal/ctxkey"
	"learn/go/day-56/internal/domain"
	"learn/go/day-56/internal/service"
	"learn/go/day-56/internal/validation"
)
...
func (h *BookmarkHandler) CreateBookmark(w http.ResponseWriter, r *http.Request) {
	...
	if err := validation.ValidateCreateInput(in); err != nil {
		writeProblem(w, http.StatusBadRequest, err.Error())
		return
	}

	bookmark, err := h.Svc.Create(r.Context(), user, in)
	if writeDomainError(w, err) {
		return
	}
...
func (h *BookmarkHandler) BulkCreateBookmarks(w http.ResponseWriter, r *http.Request) {
	...
	if err := validation.ValidateBulkCreateInputs(inputs); err != nil {
		writeProblem(w, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *BookmarkHandler) UpdateBookmark(w http.ResponseWriter, r *http.Request) {
	...
	in := toUpdateInput(req)
	if err := validation.ValidateUpdateInput(in); err != nil {
		writeProblem(w, http.StatusBadRequest, err.Error())
		return
	}

	bookmark, err := h.Svc.Update(r.Context(), user, id, in)
	if writeDomainError(w, err) {
		return
	}
	writeJSON(w, http.StatusOK, bookmark)

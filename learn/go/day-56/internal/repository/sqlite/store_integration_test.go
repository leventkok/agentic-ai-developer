...
import (
	...
	"learn/go/day-56/internal/db/testutil"
...
func TestStore_ForbiddenCrossUser(t *testing.T) {
	...
	created, err := repo.Create(ctx, owner, domain.CreateBookmarkInput{
		Title: "Private",
	...
	title := "Stolen"
	_, err = repo.Update(ctx, other, created.ID, domain.UpdateBookmarkInput{Title: &title})

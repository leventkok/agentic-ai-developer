# Verify layer boundaries — Day 61 capstone
...
$forbidden = 'net/http|database/sql|learn/go/day-61/internal/service|learn/go/day-61/internal/httpapi|learn/go/day-61/internal/repository'
...
$svcHits = rg '^\s+"(net/http|learn/go/day-61/internal/httpapi)"' internal/service/ --glob "*.go" --glob "!*_test.go" 2>$null
if ($svcHits) {
...
$apiHits = rg '^\s+"learn/go/day-61/internal/repository/sqlite"' internal/httpapi/ --glob "*.go" 2>$null
if ($apiHits) {
    Write-Host "FAIL:" -ForegroundColor Red
    $apiHits
    exit 1
